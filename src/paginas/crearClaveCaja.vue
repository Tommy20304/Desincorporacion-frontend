...
class manejarAdvertencias{
    //para manejar las multiples advertencias
    constructor(){
        this.clavesExistentes = null;
        this.registrosRepetidos = null;
        this.filasNoEncontradas = null;
    }
    advertencia(){
        manejarNotificaciones.activarAdvertencia = false;
        manejarNotificaciones.activarError = false;
        if(this.clavesExistentes){
            manejarNotificaciones.activarNotificacionAdvertencia("existen registros con claves existentes.Desea reemplazar la clave por esta?", 
            this.clavesExistentes);
            this.clavesExistentes = null;
            return true;
        }
        if(this.registrosRepetidos){
            manejarNotificaciones.activarNotificacionAdvertencia("se encontraron identificadores iguales en el archivo. Desea ingresar de todos modos?", 
            null, true);
            this.registrosRepetidos = null;
            return true;
        }
        if(this.filasNoEncontradas){
            manejarNotificaciones.activarNotificacionAdvertencia("Estas filas no se encontraron. Desea ingresar de todos modos", 
            this.filasNoEncontradas);
            this.filasNoEncontradas = null;
            return true;
        }
...
const datosEncontrados = ref(null);
const manejarAdvertencia = new manejarAdvertencias();

//se determina errores
const determinarError = async () => {
    if(!file) return;
    ...
    datosEncontrados.value = store.respuesta.resultado.datosObtenidos;

    //se determinan las advertencias
    if(store.respuesta.filasClavesExistentes.length){
        manejarAdvertencia.clavesExistentes = store.respuesta.filasClavesExistentes
    }

    if(store.respuesta.resultado.registrosRepetidos.length > 0){
        manejarAdvertencia.registrosRepetidos = true;
    }
    if(store.respuesta.resultado.filasNoObtenidas.length > 0){
        manejarAdvertencia.filasNoEncontradas = store.respuesta.resultado.filasNoObtenidas
    }

    //se determina cual advertencia se pone
    if(manejarAdvertencia.advertencia()) return;
...
<template>
    ...
    <main class="content">
      <div class="upload-card">
        ...
        <div class="actions">
          <button 
            class="btn-submit" 
            :disabled="!clave || procesar"
            @click="determinarError"
          >
  ...
  <notificacionAdv 
      :activar="manejarNotificaciones.activarAdvertencia" 
      :columnas="manejarNotificaciones.columnasAdvertencia" 
      @cerrar="manejarNotificaciones.activarAdvertencia = false" 
      :texto="manejarNotificaciones.texto"
      :preguntar="manejarNotificaciones.preguntarAdv"
      @respuesta="respuesta"
    />
  <notificacionError :activar="manejarNotificaciones.activarError" :texto="manejarNotificaciones.texto"  @cerrar="manejarNotificaciones.activarError = false"/>
   ...
   <notificacionExito :activar="manejarNotificaciones.activarExito" :texto="manejarNotificaciones.texto"  @cerrar="manejarNotificaciones.activarExito = false"/>
</template>
