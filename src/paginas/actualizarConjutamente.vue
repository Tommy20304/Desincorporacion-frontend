  ...
  <div v-if="actualizar.activarPagina" class="page-container">
    ...
    <div class="form-card">
      <header class="form-header">
        <h2>Actualizar Conjuntamente</h2>
        <p>Actualiza todos los datos de una misma manera</p>
      </header>

      <form @submit.prevent="actualizarConjuntamente" class="equipment-form">
        <div class="grid-layout">
          ...
          <div class="form-group relative">
            ...
            />
            <ul v-if="gestionOpciones.showSug.modelo && gestionOpciones.modelosFiltrados.length" class="custom-datalist">
...
const ruta = useRoute();
const datosActulizar = JSON.parse(localStorage.getItem(ruta.params.parametro+"-conjunto")); //los datos que se van a actualizar
...
async function actualizarConjuntamente(){
    //se determina que se haya ingresado un dato
    const ningunDato = Object.values(actualizar.equipo).every(item => !item || (Array.isArray(item) && item.length === 0));
    if(ningunDato){
        actualizar.enviando = false;
        actualizar.control.activarNotificacionError("No se Ingreso ningun dato");
        return;
    }

    // se determina si hay componentes repetidos
    const errorRepetidos = actualizar.comprobarComponentesRepetidos();
    if (errorRepetidos) {
      actualizar.enviando = false;
      actualizar.control.activarNotificacionError(errorRepetidos);
      return;
    }

    // se determina si la etiqueta es valida
    if (!actualizar.comprobarEtiqueta()){
      actualizar.enviando = false;
      actualizar.control.activarNotificacionError("La etiqueta no es valida");
      return;
    }

    // se determina si la mac_addre es valida
    if (!actualizar.comprobarMac()){
      actualizar.enviando = false;
      actualizar.control.activarNotificacionError("La dirección mac no es valida");
      return;
    }

    //se determina si los datos unicos de etiqueta y serial ya existen
    await actualizar.store.mandarJson({datosIngresados: actualizar.equipo}, "ingresar/verificar-campos");

    if (actualizar.store.respuesta.mensaje){
      let campos = actualizar.store.respuesta.campos;
      //si tiene datos se muestra la notificacion
      if(campos.length){
        actualizar.enviando = false;
        actualizar.control.activarNotificacionAdvertencia(`Los datos de estos campos unicos ya existen: ${campos.join(", ")}`);
        return;
      }  
    }

    const determinar = actualizar.determinarIngreso()
    if (determinar){
      //se señala que los datos no estan registrados
      actualizar.enviando = false;
      actualizar.control.activarNotificacionAdvertencia(`Los datos de estos campos no estas registrados: ${Object.keys(determinar).join(", ")}`);
      return;
    } 

    //se saca el id de los datos a actualizar
    const datosActualizarLimpio = Array.isArray(datosActulizar) ? datosActulizar : [datosActulizar];
    const IdActualizar = datosActualizarLimpio.map(row => row.id_equipo);

    //se hace la peticion
    await actualizar.store.mandarJson({ids: IdActualizar, datosActualizar: actualizar.equipo}, "actualizar/actualizar-conjunto")

    //si hay errores
    if(!actualizar.store.respuesta.mensaje){
        actualizar.enviando = false;
        actualizar.control.activarNotificacionError("Hubo errores en las actualizaciones");
        return;
    }

    actualizar.enviando = false;
    actualizar.activarPagina = false;
    actualizar.control.activarNotificacionExito("Se actualizaron los datos correctamente");
}
